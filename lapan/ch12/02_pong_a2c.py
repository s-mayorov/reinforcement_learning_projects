    ...
    with common.RewardTracker(writer, stop_reward=18) as tracker:
        with ptan.common.utils.TBMeanTracker(writer, batch_size=10) as tb_tracker:
            for step_idx, exp in enumerate(exp_source):
                ...
                states_v, actions_t, val_ref_v = unpack_batch(batch, net, device=device)
                batch.clear()
                optimizer.zero_grad()
                logits_v, value_v = net(states_v)
                loss_value_v = F.mse_loss(value_v.squeeze(-1), val_ref_v)

                log_prob_v = F.log_softmax(logits_v, dim=1)
                adv_v = val_ref_v - value_v.detach()
                ...
                loss_v = entropy_loss + loss_value_v
                loss_v.backward()
                ...
                tb_tracker.track("batch_rewards",   val_ref_v, step_idx)
