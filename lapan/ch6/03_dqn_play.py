...
if __name__ == '__main__':
    ...
    parser.add_argument("-e", "--env", default=DEFAULT_ENV_NAME, help=f"Env to use, default is {DEFAULT_ENV_NAME}")
    ...
    while True:
        ...
        if done: break
        if args.vis:
            delta = 1/FPS - (time.time() - ts)
            if delta > 0:
                time.sleep(delta)

    print(f"Total reward is {total_reward:.2f}")
